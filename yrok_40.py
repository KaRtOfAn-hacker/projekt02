...
grid = GridSearchCV(RandomForestClassifier(random_state=42),{"n_estimators":[50,100,200],"max_depth":[5,10,20,None],"min_samples_split":[2,5,10]},cv=3,scoring="f1")
grid.fit(Xtrc, ytrc)
...
plt.title("Feature Importance"); plt.tight_layout(); plt.savefig('fotog/urok_40_feature_importance.png', dpi=300, bbox_inches='tight'); plt.close()
for i,ix in enumerate(idx[:5]): print(f"{i+1}. {X.columns[ix]}: {rf.feature_importances_[ix]:.4f}")
plt.figure(figsize=(10, 8))
for n,m in models.items():
    m.fit(Xtrc, ytrc)
    fpr,tpr,_ = roc_curve(ytec, m.predict_proba(Xtec)[:,1])
    plt.plot(fpr, tpr, label=f'{n} (AUC={auc(fpr,tpr):.2f})')
plt.plot([0,1],[0,1],'k--'); plt.title('ROC Curves'); plt.legend(); plt.savefig('fotog/urok_40_roc_curves.png', dpi=300, bbox_inches='tight'); plt.close()

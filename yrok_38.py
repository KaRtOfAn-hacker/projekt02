...
df_encoded = pd.get_dummies(df, columns=['continent'], prefix='continent', dtype=int)
...
df_encoded.to_csv('covid_data_cleaned.csv', index=False)

cm = df_encoded[["new_cases","new_deaths","total_cases","population","gdp_per_capita"]].corr()
print(cm)

fig, axes = plt.subplots(2, 2, figsize=(16, 12))
sns.heatmap(cm, annot=True, fmt='.3f', cmap='coolwarm', vmin=-1, vmax=1, linewidths=1, square=True, ax=axes[0,0])
sns.boxplot(data=df, x='continent', y='total_deaths_per_million', hue='continent', ax=axes[0,1], legend=False)
sns.pairplot(df[["total_cases","total_deaths","total_vaccinations","population"]].sample(min(1000, len(df))))
plt.suptitle('Pair Plot', y=1.02); plt.savefig('fotog/urok_38_pairplot.png', dpi=300, bbox_inches='tight'); plt.close()

axes[1,0].scatter(df['total_cases'], df['total_deaths'], alpha=0.5, color='teal')
df[df["date"]==df["date"].max()].nlargest(10, "total_cases_per_million")[["location", "total_cases_per_million"]].plot(kind="bar", x="location", y="total_cases_per_million", ax=axes[1,1])

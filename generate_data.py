...
for country, iso, continent, pop, gdp in countries:
    ...
    for i, date in enumerate(dates):
        n_cases = int(np.random.poisson(lam=500 * (1 + 0.05 * i)))
        n_deaths = int(n_cases * np.random.uniform(0.01, 0.03))
        c_cases += n_cases
        c_deaths += n_deaths
        c_vac += int(np.random.poisson(lam=1000 * (1 if i > 30 else 0)))
...
df.loc[df.sample(frac=0.03, random_state=42).index, "new_cases"] = np.nan
df.loc[df.sample(frac=0.03, random_state=24).index, "new_deaths"] = np.nan
df.loc[df.sample(frac=0.03, random_state=10).index, "total_cases"] = np.nan

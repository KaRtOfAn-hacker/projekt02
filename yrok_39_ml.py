...
cls_models = {"LogReg": LogisticRegression(max_iter=1000, random_state=42), "DT": DecisionTreeClassifier(random_state=42), "RF": RandomForestClassifier(random_state=42), "kNN": KNeighborsClassifier()}
cls_res = [[n, accuracy_score(ytec, m.fit(Xtrc, ytrc).predict(Xtec)), precision_score(ytec, m.fit(Xtrc, ytrc).predict(Xtec), zero_division=0), recall_score(ytec, m.fit(Xtrc, ytrc).predict(Xtec), zero_division=0), f1_score(ytec, m.fit(Xtrc, ytrc).predict(Xtec), zero_division=0)] for n, m in cls_models.items()]
...
plt.tight_layout(); plt.savefig('fotog/urok_39_confusion_matrices.png', dpi=300, bbox_inches='tight'); plt.close()
reg_models = {"Linear": LinearRegression(), "Poly": make_pipeline(PolynomialFeatures(degree=2), LinearRegression()), "Ridge": Ridge(alpha=10.0, random_state=42, solver='auto'), "Lasso": Lasso(alpha=0.1, max_iter=5000, random_state=42)}
reg_res = [[n, mean_squared_error(yter, m.fit(Xtrr, ytrr).predict(Xter)), np.sqrt(mean_squared_error(yter, m.fit(Xtrr, ytrr).predict(Xter))), mean_absolute_error(yter, m.fit(Xtrr, ytrr).predict(Xter)), r2_score(yter, m.fit(Xtrr, ytrr).predict(Xter))] for n, m in reg_models.items()]
...
plt.tight_layout(); plt.savefig('fotog/urok_39_regression_plots.png', dpi=300, bbox_inches='tight'); plt.close()
print("Готово!")